...
class FeatureExtractor(object):
    ...
    def run(self, img: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            # Allows cpu only if needed
            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            feat = self.model(batch.to(device))
...
class VideoPreprocessor(object):
    ...
    def kts(self, n_frames, features):
        ...
        # https://github.com/li-plus/DSNet/issues/19
        if len(change_points) <= 1:
            print("Only had 1 change point, creating fixed, backup change points")
            # change_points, _ = cpd_nonlin(kernel, <choose_your_number_of_change_points>, 1, <choose_your_max_number_of_frames_per_segment>, verbose=False)
            change_points, _ = cpd_nonlin(kernel, n_frames/100, 1, n_frames, verbose=False)
